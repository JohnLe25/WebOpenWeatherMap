...
# Tùy chỉnh giao diện
st.set_page_config(page_title="Dự Báo Thời Tiết", layout="wide")
st.markdown(
    """
    <style>
        body {
...
def get_weather(city):
    ...
    if response.status_code == 200:
        ...
        for day in forecast_days:
            ...
            # Chỉnh màu nhiệt độ
            temp_color = "#66B2FF" if temp < 15 else "#66CC99" if temp <= 25 else "#FF6666"
            
            col1, col2, col3, col4 = st.columns([2, 1, 1, 2])
            col1.write(f"📅 **{date}**")
            col2.image(icon_url, width=40)
            col3.markdown(f"<span style='color:{temp_color}; font-weight:bold;'>🌡️ {temp}°C</span>", unsafe_allow_html=True)
...
def plot_temperature_chart(city, dates, temps):
    ...
    plt.plot(dates, temps, marker='o', linestyle='-', color='#FF9966', linewidth=2, label=city)
